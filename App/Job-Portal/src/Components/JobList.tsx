...
function JobList(){
    return(
        <div className="flex-grow p-8 bg-[#1a1a2e]">
            ...
            {jobs.map((job, idx) => (
                <div key={idx} className="bg-[#243146] p-5 rounded-lg shadow-md mb-4 border-black text-amber-50">
                    <div className="flex justify-between items-start">
                        <div>
                            <h4 className="text-lg font-semibold text-left"> {job.role} </h4>
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex space-x-2 mt-4">
                            {job.tags.map((tag, tagIndex) => (
                                <span key={tagIndex} className="bg-gray-800 text-white text-xs py-1 rounded-md p-2 border border-cyan-900">{tag}</span>
                            ))}    
                        </div>
